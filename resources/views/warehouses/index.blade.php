        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full whitespace-nowrap">
                    <thead
                        class="bg-slate-50 border-b border-slate-100 text-slate-600 text-xs font-semibold uppercase tracking-wider">
                        <tr>
                            <th class="text-left px-6 py-4">Warehouse Details</th>
                            <th class="text-left px-6 py-4">Assigned Suppliers</th>
                            <th class="text-left px-6 py-4">Code</th>
                            <th class="text-left px-6 py-4">Contact Phone</th>
                            <th class="text-left px-6 py-4">Status</th>
                            <th class="text-center px-6 py-4">Live Inventory & Operations</th>
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600 group-hover:bg-slate-900 group-hover:text-white transition-all">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                                                <i class="fa-solid fa-location-dot"></i> {{ $warehouse->address ?? 'Primary Storage' }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1.5 max-w-xs whitespace-normal">
                                        @forelse($warehouse->suppliers as $supplier)
                                            <span class="inline-flex items-center bg-slate-100 border border-slate-200 text-slate-800 text-xs px-2.5 py-0.5 rounded-md font-medium">
                                                 {{ $supplier->name }}
                                            </span>
                                        @empty
                                            <span class="text-xs text-slate-400 italic">No assigned suppliers</span>
                                        @endforelse
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 font-mono font-medium text-slate-600">
                                    <span
                                        class="bg-slate-100 px-2 py-1 rounded-md text-xs border border-slate-200/60">{{ $warehouse->code }}</span>
                                </td>

                                <td class="px-6 py-4 text-slate-600">
                                    @if ($warehouse->phone)
                                        <span class="flex items-center gap-1.5"><i
                                                class="fa-solid fa-phone text-xs text-slate-400"></i>
                                            {{ $warehouse->phone }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    @if ($warehouse->status)
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-xs font-medium border border-emerald-200">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                            Active
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-rose-50 text-rose-700 px-3 py-1 rounded-full text-xs font-medium border border-rose-200">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col sm:flex-row items-center justify-center gap-2">
                                        <a href="{{ route('warehouses.show', $warehouse->id) }}"
                                            class="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white font-medium px-3 py-1.5 rounded-xl text-xs transition-all border border-blue-100 shadow-sm w-full sm:w-auto justify-center">
                                            <i class="fa-solid fa-boxes-stacked"></i> View Stock
                                        ...
                                        @can('manage inventory')
                                            <a href="{{ route('inventory.transfer.create', ['from_warehouse_id' => $warehouse->id]) }}"
                                                class="inline-flex items-center gap-1.5 bg-slate-50 text-slate-700 hover:bg-slate-900 hover:text-white font-medium px-3 py-1.5 rounded-xl text-xs transition-all border border-slate-200 shadow-sm w-full sm:w-auto justify-center"
                                                title="Initiate Stock Transfer">
                                                <i class="fa-solid fa-arrow-right-arrow-left"></i> Transfer Stock
                                            </a>
                                        @endcan
                                    </div>
                                </td>
                                ...
                                <td colspan="7" class="text-center py-12 text-slate-400">
                                    <div class="flex flex-col items-center justify-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-400 mt-0.5">Try adjusting your search query or add
                                                a new location structure.</p>
                                        </div>
                                    </div>
                                </td>
                            </tr>
