...
@section('content')
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-slate-700 mb-1">No Brands Found</h3>
                                    <p class="text-sm text-slate-500">Create your first brand to start categorizing products</p>
                                    <a href="{{ route('brands.create') }}" 
    ...
    <div class="mt-6 flex items-center justify-between">
        <p class="text-sm text-slate-500">
            Showing <span class="font-semibold text-slate-700">{{ $brands->firstItem() ?? 0 }}</span> 
            to <span class="font-semibold text-slate-700">{{ $brands->lastItem() ?? 0 }}</span> 
            of <span class="font-semibold text-slate-700">{{ $brands->total() }}</span> brands
        </p>
        {{ $brands->links() }}
    </div>
@endsection
...
@push('styles')
<style>
    /* Pagination styling - matches modern UI */
    .pagination {
        display: flex;
        gap: 0.5rem;
        align-items: center;
